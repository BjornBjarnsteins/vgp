...
function handleMouse(evt) {
    ...
    // I don't remember which of the spaceships was supposed to be moved
	// when you click (and I can't seem to find it anywhere in the specifications,
	// but surely it can be any one of them, right?
	g_extraShip2.cx = x;
	g_extraShip2.cy = y;
}
...
Sprite.prototype.drawWrappedCentredAt = function (ctx, cx, cy, rotation) {
    ...
    var wrapCoordinates = getWrapCoordinates(this.image, cx, cy);

    this.drawCentredAt(ctx, wrapCoordinates.x0,
                            wrapCoordinates.y0,
                            rotation);
	...
	this.drawCentredAt(ctx, cx,
					  		wrapCoordinates.y0,
					  		rotation);
};

function getWrapCoordinates(image, cx, cy) {
	var wrapCoordinates = {x0: cx, y0: cy, x1: cx, y1: cy};

    if (cx < image.width) {
        wrapCoordinates.x0 += g_canvas.width;
    } else if (cx > g_canvas.width-image.width) {
        wrapCoordinates.x0 -= g_canvas.width;
    }

    if (cy < image.height) {
        wrapCoordinates.y0 += g_canvas.height;
    } else if (cy > g_canvas.height-image.height) {
        wrapCoordinates.y0 -= g_canvas.height;
    }
...
Ship.prototype.applyAccel = function (accelX, accelY, du) {
    ...
    if (g_useGravity && this.cy + g_shipSprite.image.height/2 > g_canvas.height) {
    	this.velY *= -0.9;
    }

    this.cx += (this.velX + oldVelX) / 2 * du;
    this.cy += (this.velY + oldVelY) / 2 * du;
...
function processDiagnostics() {
    if (eatKey(KEY_EXTRAS)) {
        ...
        g_extraShip1.active = g_useExtras;
        g_extraShip2.active = g_useExtras;
    }
...
function render(ctx) {
    ...
    // Draw ships
    g_ship.render(g_ctx);
    g_extraShip1.render(g_ctx);
    g_extraShip2.render(g_ctx);

    // sprite test
    // g_shipSprite.drawCentredAt(ctx, 150, 30, 1);
